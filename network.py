...
class ResNetV2(keras.Model):
    ...
    def mc_predict_with_sample(self, X, mask=None):
        mc_predictions = []
        ...
        for i in range(self.mc_sampling):
            sample_mis, count = sample_misspecification(mc_predictions[i], mean, mask)
            total_count += count
            total_misspecification += sample_mis
        if total_count<1:
            total_misspecification = 0.0
        else:
            total_misspecification /= float(total_count)
        try:
            total_misspecification = tf.math.sqrt(tf.cast(total_misspecification, dtype=tf.float32))
        except:
            total_misspecification = math.sqrt(float(total_misspecification))
        return mc_predictions, mean, total_misspecification
    
    def mc_predict_without_sample(self, X, mask=None):
        ...
        for i in range(self.mc_sampling):
            ...
            sample_mis, count = sample_misspecification(pred, mean, mask)
            total_count += count
            total_misspecification += sample_mis
        if total_count<1:
            total_misspecification = 0.0
    ...
    def mc_predict(self, X, mask=None, return_all=False):
        if return_all:
            return self.mc_predict_with_sample(X, mask)
        else:
            out = self.mc_predict_without_sample(X, mask)
