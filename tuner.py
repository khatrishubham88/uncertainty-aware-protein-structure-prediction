...
def main():
    def model_builder(hp):
        ...
        #hp_learning_rate = hp.Choice('learning_rate', values = [3e-3, 6e-3, 3e-4])
        #hp_reg_strength = hp.Choice('reg_strength', values = [1e-2, 1e-3, 1e-4])
        #hp_dropout_rate = hp.Choice('dropout_rate', values = [0.15, 0.1, 0.2])
        hp_kernel_initializer = hp.Choice('kernel_initializer', values=["random_normal", "glorot_uniform", "he_normal"])
        ...
        nn = ResNet(input_channels=inp_channel, output_channels=64, num_blocks=num_blocks, num_channels=num_channels,
                ...
                dropout_rate=0., reg_strength=0., kernel_initializer=hp_kernel_initializer)
        model = nn.model()

        model.compile(optimizer=tf.keras.optimizers.Adam(amsgrad=True, learning_rate=6e-2),
    ...
    # Initialze keras tuner
    tuner = kt.Hyperband(model_builder, objective='val_accuracy', max_epochs=params['epochs'], hyperband_iterations=1)
